...
class DisplayPet extends Controller
{
	 ...
	 */
	public function nextImagePost($image_id){
		$animal_id = DB::table('images')->where('image_id', $image_id)->first();
		//Get all the image ids for the images of this animal
		$image_ids = DB::table('images')->where('ref_id', $animal_id->ref_id)->where('ref_type', 'PET')->get('image_id');
		...
		return back()->with('last_image', $last_image)->with('first_image', $first_image)
				...
				->with('image_num', $animal_id->ref_id);
	}
